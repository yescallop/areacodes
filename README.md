...
- 一条记录若为省级，其“二级行政区”为空；若为地级，其“二级行政区”即为其名称；若为县级，其“二级行政区”为其启用时对应的上级行政区名称（若上级代码存在），或“直辖”（若上级代码不存在）。
    ...
    注：先前的版本中“直辖”原为“直管”。
- 一条记录的状态若为“变更”或“弃用”，其“新代码”为变更或弃用后第一年的数据中，其原行政区域所对应的代码（新代码与旧代码一致时略去）；若为“启用”，其“新代码”为其行政区域变更后第一年的数据中，原行政区域所对应的代码（不含其自身的代码）。

    注：“新代码”中代码以字符 `;` 分隔。
- 查询一条记录对应的最新代码的方法是，以该记录为根节点，按“新代码”字段展开树节点，直至所有叶节点的状态均为“启用”为止。
